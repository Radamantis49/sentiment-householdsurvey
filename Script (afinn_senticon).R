...
##MLSenticon
##Procesamiento del diccionario de MLSenticon, bajo el mismo formato de Sentiwordnet 3.0
x1 <- read_xml("senticon.es.xml")

#sacar todas las palabras
recs <- xml_find_all(x1, "//lemma")

#dejar las palabras sin espacios 
Palabra <- trimws(xml_text(recs))

#extraer los atributos y dejarlos sin espacios
pol <- trimws(xml_attr(recs, "pol"))
pos <- trimws(xml_attr(recs, "pos"))
std <- trimws(xml_attr(recs, "std"))

#dataframe
data <- as.data.frame(Palabra)
data$categoria <- pos
data$polaridad <- pol
data$sd <- std
...
#Gráfico por categoría por preguntas
cat_desocu_afinn <- filter(merge_9_final_afinn, Categoria == "Desocupados")
ggplot(data=cat_desocu_afinn) +
  aes(Numero_pregunta, Puntaje_pregunta, fill=Tipo) + 
  geom_col()
...
sa_geih <- 
  merge_9 %>%
  unnest_tokens (input = "Pregunta", output = "Palabra") %>%
  inner_join(data, ., by = "Palabra") %>%
  mutate(Tipo = ifelse(polaridad==0, "Neutro", ifelse(polaridad > 0, "Positiva", "Negativa")))

merge_9_final <-
  sa_geih %>% 
  group_by(Numero_pregunta) %>%
  summarise(Puntaje_pregunta = mean(polaridad)) %>%
  left_join(merge_9, ., by = "Numero_pregunta") %>%
  mutate(Puntaje_pregunta = ifelse(is.na(Puntaje_pregunta), 0, Puntaje_pregunta))
...
#Mapeo de positivos negativos por preguntas

map(c("Positiva", "Negativa", "Neutro"), function(sentimiento) {
  merge_9_final %>%
    filter(Tipo == sentimiento) %>%
    group_by(Categoria) %>%
    count(Numero_pregunta, sort = T) %>%
    top_n(n = 5, wt = n) %>%
    ggplot() +
    aes(Numero_pregunta, n, fill = Categoria) +
    geom_col() +
    facet_wrap("Categoria", scales = "free") +
...
#Gráfico por categoría por preguntas
cat_ocu <-  filter(merge_9_final, Categoria == "Ocupados")
cat_desocu <- filter(merge_9_final, Categoria == "Desocupados")

ggplot(data=cat_desocu) +
aes(Numero_pregunta, Puntaje_pregunta, fill=Tipo) + 
geom_col() 

#Exportar

write_excel_csv(merge_9_final, "/home/morracos/Documentos/R_scraping/GEIH_SA/base_marcada.csv", delim = ",")
